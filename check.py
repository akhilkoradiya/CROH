...
def banner():
	print('')
	print('python3 check.py <cURL options> <target>')
	print('')
	print('curl options :')	
	print(' -L follow redirections 30X responses')
	print(' -x <ip>:<port> to set a proxy') 
	print(' -m <seconds> to set a timeout')
	print(' -H for new headers')
	print(' -d for data in the POST requests body')
	print('')
...
def curl_code_response(options_var, payload_var):
	...
	#status code -> 200 color -> Green
	#status code -> 30X color -> Orange
	#4status code -> 0X or 50X ->Red
